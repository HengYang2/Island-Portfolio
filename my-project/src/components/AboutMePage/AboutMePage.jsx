    ...
    function toggleQuestionsVisable() {
        return questionsVisable ?
            <div className='w-40 h-36 ml-90 mt-40 absolute flex flex-col justify-center gap-1 items-center'>
                ...
                <button className='text-option' id='option1' onClick={e => { setSelectedQuestion('option1'); setQuestionsVisable(false) }}>Who are you?</button>
                <button className='text-option' id='option2' onClick={e => { setSelectedQuestion('option2'); setQuestionsVisable(false) }}>Where am I?</button>
                <button className='text-option' id='option3' onClick={e => { setSelectedQuestion('option3'); setQuestionsVisable(false) }}>What do you do for fun?</button>
            </div> :
            <></>;
    }
    ...
    return (
        <div className='absolute top-0 left-0 w-full h-full pt-16'>
            ...
            <div className='relative w-3/4 h-44 ml-40 mt-80 flex flex-row justify-center gap-2'>
                ...
                <p onClick={() => { setSelectedQuestion(''); setQuestionsVisable(true) }} className='relative bg-white left-0 w-4/5 top-1.5 h-40 p-2 border border-black rounded-md box-shadow'>{responseModule(selectedQuestion)}</p>
