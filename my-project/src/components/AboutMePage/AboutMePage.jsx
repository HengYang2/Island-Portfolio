...
export default function AboutMePage() {
    ...
    function conditionallyRenderText() {
        switch (selectedQuestion) {
            case 'option1':
                return <p onClick={() => { setSelectedQuestion(''); setQuestionsVisable(true) }} className='relative bg-green-400 left-0 w-4/5 top-1.5 h-40 p-2 border border-black rounded-md'>My name is Heng Yang, I'm a software developer who enjoys creating games, making music and chilling on this island.</p>
            case 'option2':
                return <p onClick={() => { setSelectedQuestion(''); setQuestionsVisable(true) }} className='relative bg-green-400 left-0 w-4/5 top-1.5 h-40 p-2 border border-black rounded-md'>You are on Paradise Island, a place created from my imagination. You are expereiencing a piece of my mind!</p>
            case 'option3':
                return <p onClick={() => { setSelectedQuestion(''); setQuestionsVisable(true) }} className='relative bg-green-400 left-0 w-4/5 top-1.5 h-40 p-2 border border-black rounded-md'>I enjoy making music, 3d modeling, coding, and spending time with friends and family.</p>
            default:
                return <p onClick={() => { setSelectedQuestion(''); setQuestionsVisable(true) }} className='relative bg-green-400 left-0 w-4/5 top-1.5 h-40 p-2 border border-black rounded-md'>You seem a little lost stranger, is there something I can help you with?</p>
        }
